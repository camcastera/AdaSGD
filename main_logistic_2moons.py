...
######
## Define the dataset
pbname = 'logreg_2moons'


A_temp, b_moon = make_moons(n_samples=200, noise=0.2, random_state=42)
A_moon = torch.ones(A_temp.shape[0], A_temp.shape[1] + 1)
A_moon[:, 1:] = torch.tensor(A_temp) ; 
b_moon[b_moon == 0] = -1. #switch to -1, 1 formulation
b_moon = torch.tensor(b_moon)

N_samples, n = A_moon.shape[-2:]

fig, ax = plt.subplots(figsize=(6, 4))
ax.scatter(A_moon[b_moon == -1][:, 1], A_moon[b_moon == -1][:, 2], color='blue')
ax.scatter(A_moon[b_moon == 1][:, 1], A_moon[b_moon == 1][:, 2], color='red')


oracle_logistic = lambda x, minibatch: logistic_forward(x, A_moon, b_moon, minibatch=minibatch)
