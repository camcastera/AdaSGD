...
def Poisson_forward(X, A, b, minibatch=None):
    ...
    g = 1 / len(minibatch) * torch.sum(torch.exp(correl) - b[minibatch] * correl, dim=-1) #+ regul #returns one value per data sample
    return g
...
######
## Define the dataset
pbname = 'Poisson_synthetic'

A_poisson = torch.tensor(np.load('datasets/poisson_A.npy')).float()
b_poisson = torch.tensor(np.load('datasets/poisson_b.npy')).float()
x0_poisson = torch.tensor(np.load('datasets/poisson_x0.npy')).float()

N_samples, n = A_poisson.shape[-2:]

oracle_Poisson = lambda x, minibatch: Poisson_forward(x, A_poisson, b_poisson, minibatch=minibatch)

Poisson_func = objective_funcs.obj_func(oracle_Poisson)
######
#Load fstar
...
for i, ((key, stepsize), color, zorder) in enumerate(zip(list_best_keys, list_colors, list_zorders)):
    list_f = np.concatenate(([Poisson_func.eval_f(optim_parameters['x0'])], Dict_best_results[(key, stepsize)][1]))
