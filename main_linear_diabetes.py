...
######
## Define the dataset
pbname = 'diabetes_linear'

from sklearn.datasets import load_diabetes
diabetes = load_diabetes()
A_diabetes, b_diabetes = torch.tensor(diabetes.data).float(), torch.tensor(diabetes.target).float()
#P = A.shape[1]

N_samples = A_diabetes.shape[0]
n = A_diabetes.shape[1]


# Below creates a forward function that only takes x and minibatch as input 
oracle_quad = lambda x, minibatch: quadratic_forward(x, A=A_diabetes, b=b_diabetes, minibatch=minibatch)
...
xstar = 0 - torch.linalg.solve(1 / N_samples * A_diabetes.T @ A_diabetes, quad_func.eval_gradf(torch.zeros(n)))
fstar = quad_func.eval_f(xstar).item()
fstar_diabetes = fstar
